...
import RequireAuth from 'web/components/auth/RequireAuth';
import Page404 from 'web/pages/404';
    ...
    <Suspense>
      <Routes>
        ...
        {privateRoutes.map(({ element, ...privateRouteProps }) => (
          <Route element={<RequireAuth redirectTo={`/login?redirectTo=${privateRouteProps.path}`}>{element}</RequireAuth>} {...privateRouteProps} key={`privateRoute-${privateRouteProps.path}`} />
        ))}
