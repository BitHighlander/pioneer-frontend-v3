...
const Header = () => {
    ...
    try {
      if (user && user.wallets) {
        ...
        for (let i = 0; i < pubkeys.length; i++) {
          ...
          const walletType = walletDescriptions.filter((wallet: { context: any }) => wallet.context === context)[0]?.type;
  ...
  const renderPubkeysPagination = pubkeysPageNumbers.map((pageNumber) => (
    <Button key={pageNumber} onClick={() => handlePubkeysPageClick(pageNumber)} colorScheme={pageNumber === pubkeysCurrentPage ? 'teal' : 'gray'} variant="outline" size="sm" mx="1">
      {pageNumber}
    </Button>
  ));
              ...
              <TabPanel>
                ...
                <Stack spacing="2">
                  {currentPubkeys.map((pubkey: any, index: number) => (
                    <Card key={pubkey.pubkey} onClick={() => handleCardClick(pubkey.pubkey)} variant="elevated" maxW="sm" borderWidth="1px" borderRadius="md" boxShadow="md" _hover={{ boxShadow: 'lg' }}>
                      <CardBody>
                        <Flex align="center">
                          ...
                          <Box display="block" overflowY="scroll">
                            ...
                            <Button size="xs" onClick={(event) => handleCopyClick(event, pubkey.master)} marginTop="1">
